...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const excludedDomains = [
    "agoda", "booking", "trivago", "expedia", "zenhotels", "skyscanner",
    "airpaz", "readytotrip", "lodging-world", "yatra", "rentbyowner",
    "goibibo", "laterooms", "tiket", "tripadvisor",
  ];
  ...
  searchButton.addEventListener("click", async () => {
    ...
    reader.onload = async (e) => {
      ...
      for (const row of jsonData) {
        ...
        if (hotelUrlType && hotelUrlType.trim().toLowerCase() === "ctrip superagg") {
          query += " trip";
          requireTripDomainOnly = true;
        }

        const searchURL = `${endpoint}?q=${encodeURIComponent(query)}&format=json`;
        let matchedLink = [];

        try {
          const response = await fetch(searchURL, { method: "GET", mode: "cors", credentials: "include", headers: { "Content-Type": "application/json", Accept: "application/json" } });
          ...
          if (resultsFromXNG && resultsFromXNG.length > 0) {
            ...
            for (const result of resultsFromXNG) {
              ...
              if (!excludedDomains.some((d) => pageUrl.includes(d)) && (!requireTripDomainOnly || pageUrl.includes("trip")) && (requireTripDomainOnly || hotelNameArray.some((p) => rootDomain.includes(p)))) {
                const match = isHotelNameInPage(hotelNameArray, pageTitle);
                if (match.status) {
                  const matchScore = countHotelNameMatches(hotelNameArray, rootDomain);
                  resultsArray.push({ percentage: match.percentage, matchedLink: pageUrl, rootDomain, matchScore });
                }
              }
            }
  ...
  if (downloadCSVButton) {
    downloadCSVButton.addEventListener("click", () => {
      ...
      link.download = "room_xng_hotel_search_results.csv";
      link.click();
