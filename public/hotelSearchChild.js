...
document.addEventListener("DOMContentLoaded", function () {
  ...
  searchButton.addEventListener("click", async () => {
    ...
    if (!subscriptionKey) {
      if (typeof Toasts !== "undefined") Toasts.show("Vui lòng nhập Bing API Key!", { type: "warning", title: "Thiếu API Key" });
      return;
    }
        ...
        try {
          ...
          if (resultsFromBing && resultsFromBing.length > 0) {
            ...
            for (const result of resultsFromBing) {
              ...
              if (!excludedDomains.some((d) => pageUrl.includes(d)) && (!requireTripDomainOnly || pageUrl.includes("trip"))) {
  ...
  if (downloadCSVButton) {
    downloadCSVButton.addEventListener("click", () => {
      ...
      link.download = "bing_hotel_search_results.csv";
      link.click();
